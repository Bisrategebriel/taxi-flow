...
import { importFares } from "@/app/(admin)/admin/_actions/fares";

type ParsedRow = { route: string; distance_km: number; fare_etb: number };
type ImportResult = { route: string; success: boolean; error?: string };
...
function parseCSV(text: string): { rows: ParsedRow[]; error: string | null } {
  ...
  if (!headers.includes("route") || !headers.includes("fare_etb"))
    return { rows: [], error: 'CSV must have "route" and "fare_etb" columns.' };

  const get = (vals: string[], key: string) =>
    vals[headers.indexOf(key)]?.trim().replace(/^"|"$/g, "") ?? "";

  const rows: ParsedRow[] = lines.slice(1).flatMap((line) => {
    const vals = line.split(",");
    const route = get(vals, "route");
    if (!route) return [];
    return [
      {
        route,
...
export default function ImportFaresButton() {
  ...
  function handleImport() {
    startTransition(async () => {
      const res = await importFares(rows);
  ...
  function downloadResults() {
    const lines = [
      "Route,Status,Error",
      ...results.map(
        (r) => `"${r.route}","${r.success ? "updated" : "failed"}","${r.error ?? ""}"`
      ),
    ];
    ...
    a.href = url;
    a.download = "fare-import-results.csv";
    a.click();
        ...
        >
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                <h2 className="text-sm font-semibold">Import Fares &amp; Distances</h2>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {step === "upload" && "Upload CSV to bulk-update fares and distances"}
                  {step === "preview" &&
              ...
              {step === "upload" && (
                <>
                  ...
                  <div className="rounded-lg bg-muted/40 px-4 py-3 text-xs text-muted-foreground space-y-1">
                    ...
                    <p className="font-mono">route,distance_km,fare_etb</p>
                    <p>Route names must match existing routes exactly.</p>
                  </div>
                </>
              )}
                  ...
                  <div className="max-h-60 overflow-y-auto rounded-xl border border-border scrollbar-thin">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-muted/60">
                        <tr>
                          {["Route", "Distance (km)", "Fare (ETB)"].map((h) => (
                            <th
                              key={h}
