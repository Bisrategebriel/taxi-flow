...
export default function AddRouteModal({ terminals }: { terminals: TerminalOption[] }) {
  ...
  function handleOpen() {
    setError(null); setFromId(""); setToId(""); setViaIds([]); setIsActive(true);
    setOpen(true);
  }
        ...
        >
          <div className="w-full max-w-md rounded-2xl border border-border bg-card shadow-2xl max-h-[90vh] flex flex-col">
            <div className="flex items-start justify-between px-6 pt-6 pb-4 shrink-0">
              <div>
                <h2 className="text-base font-semibold">Add New Route</h2>
                <p className="text-xs text-muted-foreground mt-0.5">Configure a route between two terminals.</p>
              </div>
              <button type="button" onClick={() => setOpen(false)} className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-muted transition-colors mt-0.5">
                <X size={15} />
              </button>
            </div>

            <form ref={formRef} onSubmit={handleSubmit} className="px-6 pb-6 space-y-4 overflow-y-auto">
              {error && <p className="rounded-lg bg-destructive/10 px-3 py-2 text-xs text-destructive">{error}</p>}

              {/* Route Name */}
              <div className="space-y-1.5">
                <label className="text-xs font-medium">Route Name</label>
                <input name="name" required placeholder="e.g. Central — Airport Express"
              ...
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <input name="fare_etb" type="number" step="0.01" min="0" placeholder="0.00"
