...
import { importRoutes } from "@/app/(admin)/admin/_actions/routes";

type ParsedRow = { name: string; from_terminal: string; to_terminal: string; via: string; distance_km: number; fare_etb: number };
...
function parseCSV(text: string): { rows: ParsedRow[]; error: string | null } {
  ...
  if (!headers.includes("name") || !headers.includes("from_terminal") || !headers.includes("to_terminal"))
    return { rows: [], error: 'CSV must have "name", "from_terminal", and "to_terminal" columns.' };
  ...
  for (const line of lines.slice(1)) {
    ...
    rows.push({
      name,
      from_terminal: get(vals, "from_terminal"),
      to_terminal: get(vals, "to_terminal"),
      via: get(vals, "via"),
      distance_km: parseFloat(get(vals, "distance_km")) || 0,
      fare_etb: parseFloat(get(vals, "fare_etb")) || 0,
    });
  }
  return { rows, error: null };
}

export default function ImportRoutesButton() {
  ...
  function handleImport() {
    startTransition(async () => {
      const res = await importRoutes(rows);
      setResults(res.results); setStep("results");
    });
  }
  ...
  function downloadResults() {
    ...
    a.download = "route-import-results.csv"; a.click();
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4"
          ...
          <div className="w-full max-w-lg rounded-2xl border border-border bg-card shadow-2xl">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border">
              <div>
                <h2 className="text-sm font-semibold">Import Routes</h2>
                <p className="text-xs text-muted-foreground mt-0.5">
                  {step === "upload" && "Upload CSV to bulk-create routes"}
                  {step === "preview" && `${rows.length} route${rows.length !== 1 ? "s" : ""} ready`}
                  {step === "results" && `Done — ${successCount} imported, ${failCount} failed`}
                </p>
              </div>
              <button type="button" onClick={handleClose} className="flex h-7 w-7 items-center justify-center rounded-md text-muted-foreground hover:bg-muted transition-colors"><X size={15} /></button>
            </div>
              ...
              {step === "upload" && (
                <>
                  <div onClick={() => fileRef.current?.click()}
                    ...
                    <div className="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10"><Upload size={18} className="text-primary" /></div>
                  ...
                  <div className="rounded-lg bg-muted/40 px-4 py-3 text-xs text-muted-foreground space-y-1">
                    ...
                    <p className="font-mono">name,from_terminal,to_terminal,via,distance_km,fare_etb</p>
                    <p>Terminal names must match existing terminals exactly.</p>
                  </div>
                </>
              )}
                  ...
                  <div className="max-h-60 overflow-y-auto rounded-xl border border-border scrollbar-thin">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-muted/60">
                        <tr>{["Name", "From", "To", "Via", "km", "ETB"].map(h => <th key={h} className="text-left px-3 py-2 font-medium text-muted-foreground">{h}</th>)}</tr>
                      </thead>
                      <tbody>
                        {rows.map((r, i) => (
                          <tr key={i} className="border-t border-border hover:bg-muted/30">
                            <td className="px-3 py-2">{r.name}</td>
                            <td className="px-3 py-2">{r.from_terminal}</td>
                            <td className="px-3 py-2">{r.to_terminal}</td>
                            <td className="px-3 py-2">{r.via || "—"}</td>
                            <td className="px-3 py-2 font-mono">{r.distance_km || "—"}</td>
                            <td className="px-3 py-2 font-mono">{r.fare_etb || "—"}</td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
              ...
              {step === "results" && (
                <>
                  <div className="max-h-60 overflow-y-auto rounded-xl border border-border scrollbar-thin">
                    <table className="w-full text-xs">
                      <thead className="sticky top-0 bg-muted/60">
                        <tr><th className="text-left px-3 py-2 font-medium text-muted-foreground">Name</th><th className="text-left px-3 py-2 font-medium text-muted-foreground">Status</th></tr>
                      </thead>
                      <tbody>
                        {results.map((r, i) => (
                          <tr key={i} className="border-t border-border hover:bg-muted/30">
